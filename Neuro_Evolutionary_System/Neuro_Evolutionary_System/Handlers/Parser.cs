...
namespace Neuro_Evolutionary_System.Handlers
{
    public class Parser : IParser
    {
	    ...
	    private const string Algorithm = "Algoryth type";
	    ...
	    private const string SigmaProbabilities = "Probability of each sigma";
	    ...
	    private bool ParseLine(string line, out double[] result, out GaVariables settings)
	    {
		    ...
		    {
				var matches = Regex.Matches(line, "[0-9.]+");
				if (line.Contains(Mortality) || line.Contains(MutationProbability) || line.Contains(MinimumError))
			    {
			    ...
			    if(line.Contains(Sigmas) || line.Contains(SigmaProbabilities) || line.Contains(Architecture))
			    {
				    var success = false;
